...
export interface ObjectDeps {
  ...
  playerZ: () => number;
  playerRadius: number;
  damageArea: (x: number, z: number, radius: number, damage: number) => void; // 혈귀술 폭탄 폭발(적 피해) — run이 hash로 처리
  ...
  stunEnemies?: (x: number, z: number, radius: number, durationSec: number) => void; // #41 전집중 북 스턴
  magnetizeGems?: () => void; // #41 경보 종 젬 자석
}
...
const KIND_GONG = 4; // 경보 종 銅鑼 — 접촉 시 필드 젬 자석 (#41 15.7)
const KIND_DRUM = 5; // 전집중 북 戰鼓 — 접근 시 북 울림 스턴 (#41 15.4)
const BARREL_RADIUS = 6.5; // #41 15.6: 4→6.5
const FUSE_TIME = 3.0; // 점화 후 폭발까지(오너 피드백: 1s→3s, 전략적 여유). 연쇄 도미노는 0.3s 유지
const DRUM_CD = 25; // #41 15.4: 전집중 북 재발동 쿨다운
const HINT_RADIUS = 5.2; // 근접 힌트 표시 거리

// 종류별 이름표 / 힌트 / 이름표 높이 / 상시 글로우(r,g,b,반경, 0이면 없음)
const OBJ_NAME_KO = ['혈귀술 폭탄 爆彈', '등나무꽃 약탕 藥湯', '등나무꽃 사당 神社', '귀살대 결계목 結界', '경보 종 警鐘', '전집중 북 戰鼓'];
const OBJ_NAME_EN = ['Blood Demon Bomb', 'Wisteria Elixir Cart', 'Wisteria Shrine', 'Corps Barrier', 'Warning Bell', 'Breathing Drum'];
const OBJ_HINT_KO = ['공격 → 유폭', '회복 +30%', '호흡 가호 획득', '공격 → 파괴', '접촉 → 젬 흡수', '접근 → 적 기절'];
const OBJ_HINT_EN = ['Attack → explode', 'Heal +30%', 'Gain blessing', 'Attack → break', 'Touch → pull gems', 'Near → stun foes'];
...
const OBJ_GLOW: (readonly [number, number, number, number] | null)[] = [
  [1.5, 0.5, 0.18, 2.5], // 혈귀술 폭탄 — 위험한 붉은 잔광
  [0.55, 1.25, 0.7, 2.2], // 등나무꽃 약탕 — 온기 도는 녹빛
  [1.5, 1.05, 0.4, 3.4], // 등나무꽃 사당 — 금빛 후광(가장 크게)
  null, // 목책 — 글로우 없음
  [1.7, 1.3, 0.4, 2.6], // 경보 종 — 금빛 공명
  [1.4, 0.9, 0.5, 2.4], // 전집중 북 — 낮은 주황 진동
];

// 혈귀술 폭탄(폭발·연쇄) / 등나무꽃 약탕(회복) / 등나무꽃 사당(버프) 관리.
export class BattlefieldObjects {
  ...
  private readonly igniteT = new Float32Array(OBJ_CAP); // 혈귀술 폭탄 점화 잔여(>0=점화 중), 0=미점화, #41 15.6
  private readonly drumCd = new Float32Array(OBJ_CAP); // 전집중 북 재발동 쿨다운
  ...
  private spawnObject(gameTime: number): void {
    ...
    if (slot < 0) return;
    const r = this.d.rng;
    // 종류 확률(#41 회복 이코노미): 만두 90s+로 조기화(회복 접근성↑) + 경보 종/전집중 북 신규.
    const rv = r.next();
    // 연민 가중치: 플레이어가 저체력(<40%)이면 만두 확률을 크게 올려 회복 접근성 보장.
    const dumplingGate = (this.d.playerHpFrac?.() ?? 1) < 0.4 ? 0.6 : 0.44;
    let kind = KIND_BARREL;
    if (rv < 0.07) kind = KIND_GONG; // 경보 종 7%
    else if (rv < 0.15) kind = KIND_DRUM; // 전집중 북 8%
  ...
  // 무한성 결계(DESIGN 20): 지정 위치에 만두/경보 종 배치. 슬롯 없으면 false.
  ...
  update(dt: number, gameTime: number): void {
    ...
    // 플레이어 접촉(만두/사당/경보 종) + 근접(전집중 북) + 상시 이펙트 + 혈귀술 폭탄 점화 진행
    ...
    for (let i = 0; i < OBJ_CAP; i++) {
      ...
      // 혈귀술 폭탄 점화 상태 진행: 심지 불꽃 + 통 점멸 후 폭발
  ...
  emitMarkers(layer: MarkerLayer, px: number, pz: number): void {
    ...
    for (let i = 0; i < OBJ_CAP; i++) {
      ...
      // 상호작용 어포던스 링 — 사당/경보 종/전집중 북가 "쓸 수 있는 것"으로 읽히고 위치가 드러나게(#50 오너 Q).
      if (k === KIND_SHRINE) layer.interactRing(x, z, 1.5, 1.05, 0.4, true); // 금빛 버프
      else if (k === KIND_GONG) layer.interactRing(x, z, 1.5, 1.2, 0.5, true); // 경보 종 자석
      else if (k === KIND_DRUM) layer.interactRing(x, z, 1.4, 0.85, 0.4, this.drumCd[i] <= 0); // 전집중 북: 쿨 중이면 흐리게
      const dx = px - x;
      const dz = pz - z;
  ...
  private interact(i: number): void {
    if (this.kind[i] === KIND_DUMPLING) {
      ...
      this.d.banner(getLang() === 'en' ? 'Wisteria Elixir Cart' : '등나무꽃 약탕', '#9affc0');
      ...
      this.d.banner(`${en ? 'Wisteria Shrine' : '등나무꽃 사당'} · ${label}`, '#e8c667');
    } else if (this.kind[i] === KIND_GONG) {
      // 경보 종: 필드 전체 젬 자석 + 금빛 공명 링 2겹 (#41 15.7)
      ...
      this.d.banner(getLang() === 'en' ? 'Warning Bell' : '경보 종', '#ffd86b');
    }
    this.alive[i] = 0;
  }

  // 전집중 북: 북 울림 — 반경 내 적 0.8s 스턴 + 낮은 주황 충격 링(광원 없음, 절제). 25s 쿨.
  private beatDrum(i: number): void {
    ...
    this.d.banner(getLang() === 'en' ? 'Breathing Drum' : '전집중 북', '#e4a05b');
  }
